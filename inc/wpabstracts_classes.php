...
class WPAbstract_Abstracts_Table extends WP_List_Table {
    ...
    function column_reviewers($item){
        $reviewers = array();
        for($i = 1; $i <= 15; $i++) {
          $reviewers[] = get_userdata($item->{"reviewer_id".$i});
        }
    ...
    function column_rating($item) {
      $reviewer_ids = array();
      for($i = 1; $i <= 15; $i++) {
        $reviewer_ids[] = $item->{'reviewer_id'.$i};
      }
	...
	$screen = get_current_screen();
        ...
        $query = "SELECT abstract_id, title, event, topic, session, priority, presenter, status, author_email, ".$reviewer_select_string."submit_date, last_edit_date FROM  . $table_name";
