...
include(WPABSTRACTS_PLUGIN_DIR.'/slim/slim.php');
...
function wpabstracts_getProfileImageUrl($profile_image) {
  if($profile_image == null || strlen($profile_image) == 0 ) {
    return null;
  }
  else if(strpos($abstract[0]->profile_image, "/") !== false) {
    return $profile_image;
  }
  else {
    return WPABSTRACTS_PROFILE_IMAGE_URL.$abstract[0]->profile_image;
  }
}
/*
...
function wpabstracts_manageAbstracts($id, $data, $action){
    ...
    if($data){
        ...
        $abs_session = sanitize_text_field($data["abs_session"]);
        $abs_priority = sanitize_text_field($data["abs_priority"]);
        ...
        if (isset($_POST['slim'])) {
          $values = $_POST['slim'];
          if(!is_array($_POST['slim'])) {
            $_POST['slim'] = stripslashes($_POST['slim']);
          }
          else {
            for($i = 0; $i < count($_POST['slim']); $i++) {
              $_POST['slim'][$i] = stripslashes($_POST['slim'][$i]);
            }
          }
        }
        $images = Slim::getImages();
        if(is_array($images) && count($images) > 0 ) {
          $abs_profile_image = $images[0];
        }
        else {
          $abs_profile_image = null;
        }
    }
    switch($action){
        case 'insert':
            $image_file_name = '';
            if($abs_profile_image) {
              $extension = pathinfo($abs_profile_image['output']['name'], PATHINFO_EXTENSION);
              $image_file = Slim::saveFile($abs_profile_image['output']['data'], $abs_event.".".$extension, WPABSTRACTS_PROFILE_IMAGE_DIR);
              $image_file_name = $image_file['name'];
              wpabstracts_addProfileImageToMediaLibrary($image_file_name);
            }
            $wpdb->show_errors();
            ...
            $wpdb->insert($wpdb->prefix.'wpabstracts_abstracts',
                    array(
                       ...
                       'topic' => $data['abs_topic'], 'session' => $abs_session, 'status' => "Pending", 'author' => $abs_authors, 'author_email' => $abs_authors_email,
                'author_affiliation' => $abs_authors_affiliation, 'presenter' => $abs_presenter, 'presenter_company' => $abs_presenter_company, 'presenter_email' =>$abs_presenter_email,
                       ...
                       'manager_comments' => $manager_comments, 'profile_image' => $image_file_name
                        ));
        ...
        case 'update':
            ...
            if($abs_profile_image) {
              ...
              $image_file_name = $image_file['name'];
              wpabstracts_addProfileImageToMediaLibrary($image_file_name);
              if(strlen($original_abstract[0]->profile_image) != 0) {
                wpabstracts_removeProfileImageFromMediaLibrary($original_abstract[0]->profile_image);
              }
            }
            $date_time = current_time( 'mysql' );
            $sql = "UPDATE ".$wpdb->prefix."wpabstracts_abstracts
                        SET title = '$abs_title', text = '$abs_text', resume = '$resume_text', target_group = '$target_group', event = '$abs_event', topic = '$abs_topic', session = '$abs_session', priority = '$abs_priority',
            ...
            if($abs_profile_image && strlen($image_file_name) > 0) {
              $sql = $sql.", profile_image = '$image_file_name'";
            }
        ...
        case 'delete':
            ...
            if($profile_image != '') {
              wpabstracts_removeProfileImageFromMediaLibrary($profile_image);
            }
...
function wpabstracts_addProfileImageToMediaLibrary($filename) {
  $filetype = wp_check_filetype( basename( $filename ), null );
  $wp_upload_dir = wp_upload_dir();
  $filepath = WPABSTRACTS_PROFILE_IMAGE_DIR . basename( $filename );
  $fileurl = WPABSTRACTS_PROFILE_IMAGE_URL . basename( $filename );
  $attachment = array(
  	'guid'           => $fileurl,
...
function wpabstracts_removeProfileImageFromMediaLibrary($image_name) {
	global $wpdb;
	$attachment = $wpdb->get_col($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE guid='%s';", WPABSTRACTS_PROFILE_IMAGE_URL.$image_name ));
 ...
 */
function wpabstracts_manageProfileImage($abstract_id, $data, $action) {
  global $wpdb;
  ...
  switch($action) {
    ...
    case 'update':
      ...
      $image = $abstract[0]->profile_image;
      if(preg_match("/^\d+\.png$/", $image)) {
        $new_image = $abstract_id . "v2.png";
      }
      else if(preg_match("/^\d+v(?P<version>\d+)\.png$/", $image, $matches)){
        $new_image = $abstract_id . "v" . (intval($matches['version']) +1) . ".png";
      }
      else {
         $new_image = $abstract_id.'.png';
      }
      $a = file_put_contents(WPABSTRACTS_PROFILE_IMAGE_DIR.$new_image, $data);

      $wpdb->show_errors();
      $sql = "UPDATE ".$wpdb->prefix."wpabstracts_abstracts SET profile_image = '$new_image' WHERE abstract_id = '$abstract_id'";
      $wpdb->query($sql);
      if(file_exists(WPABSTRACTS_PROFILE_IMAGE_DIR.$image)) {
        unlink(WPABSTRACTS_PROFILE_IMAGE_DIR.$image);
      }
    break;
  }
}

function wpabstracts_manageImageUploaded($abstract_id, $image_url, $action) {
  global $wpdb;

  $sql = "UPDATE ".$wpdb->prefix."wpabstracts_abstracts SET profile_image = '$image_url' WHERE abstract_id = '$abstract_id'";
  $wpdb->query($sql);
}
...
function fixImages() {
  ...
  foreach($abstracts as $abstract) {
    if(preg_match("/^[a-zA-Z0-9]+\.png$/", $abstract->profile_image)) {
      ...
      if(file_exists($image_dir) && filesize($image_dir) > 5 * 1024) {//If larger than 5KB
        //Move to media library manually. Edit
        echo("NO: ".$image_dir);
        echo("<br/>");
      }
    }
